...
def calculate_ece(conf_mat, num_bins, name):
    ...
    return accuracy, confidence, ece, max_ce, key
...
def calculate_ece_reg(gt_list, pred_list, histogram_bin_count=15):
    ...
    for box_dim in range(all_predicted_gt.shape[1]):
        ...
        for i in torch.arange(
            ...
            # Add the bin accuracy
            acc_list.append(num_elems_in_bin_i / all_predicted_scores.shape[0])
...
def plot_reliability_clf(class_name, acc, conf, ece, max_ce, save_path):
    interval = 1 / len(acc)
